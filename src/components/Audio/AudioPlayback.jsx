...
function EndTimeValue({
    ...
    const onChangeEndTime = useCallback(function onChangeEndTime(e) {
        audioDispatch(
            audioActions.changeEndTime(id, persent(100, 100 - e.target.value, duration))
        );
    },[audioDispatch, duration, id]);
...
function StartTimeValue({
    ...
    const onChangeStartTime = useCallback(function onChangeStartTime(e) {
        audioDispatch(
            audioActions.changeStartTime(id, persent(100, e.target.value, duration))
        );
    },[audioDispatch, duration, id]);
...
function Playback({
    ...
    const startPointValue = useMemo(
        function () { return persent(duration, startPoint, 100); },
        [startPoint, duration]
    );
    const endPointValue = useMemo(
        function () { return persent(duration, endPoint, 100); },
        [endPoint, duration]
    );
